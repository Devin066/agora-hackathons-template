...
// Get repository name from event name or git
...
if (fs.existsSync('package.json')) {
  try {
    ...
    // package.json name must be lowercase and URL-safe
    pkg.name = config.repo_name.toLowerCase();
    fs.writeFileSync('package.json', JSON.stringify(pkg, null, 2));
    if (debug) console.log(`Updated package.json name to: ${pkg.name}`);
  } catch (error) {
    console.error('Error updating package.json name:', error);
  }
}

// Add calculated fields for template processing
if (config.judging_criteria) {
  if (Array.isArray(config.judging_criteria)) {
    ...
    // Handle separate phases
    if (config.judging_criteria.initial_phase) {
      config.judging_criteria.initial_phase.forEach((criteria, index) => {
        criteria.weight_decimal = (criteria.weight / 100).toFixed(2);
        criteria.loop = { index: index + 1 };
      });
    }
    if (config.judging_criteria.final_phase) {
      config.judging_criteria.final_phase.forEach((criteria, index) => {
        criteria.weight_decimal = (criteria.weight / 100).toFixed(2);
        criteria.loop = { index: index + 1 };
      });
    }
...
function updateReadme() {
  const template = fs.readFileSync('templates/template_README', 'utf8');
...
function updateRubric() {
  const template = fs.readFileSync('templates/template_hackathon-rating-rubric', 'utf8');
  let content = template;
...
function updateThoughtStarters() {
  const template = fs.readFileSync('templates/template_Thought_Starters', 'utf8');
  let content = template;
...
function updateEvaluation() {
  const template = fs.readFileSync('templates/template_evaluation', 'utf8');
  let content = template;
...
function cleanup() {
  try {
    ...
    if (fs.existsSync('templates')) {
      ...
      if (debug) console.log('Deleted: templates/');
    }
